...
function SettingsModule() {
  ...
  const refreshBackupFiles = async (target: BackupTarget) => {
    ...
    try {
      const response = await fetch(`/api/backup?action=files&id=${encodeURIComponent(target.id)}`, { headers: authHeaders() });
  ...
  const testBackupTargetAction = async (target?: BackupTarget) => {
    ...
    try {
      const response = await fetch("/api/backup?action=test", {
        method: "POST",
        headers: authHeaders(),
  ...
  const runBackupNow = async (target: BackupTarget) => {
    ...
    try {
      const response = await fetch(`/api/backup?action=run&id=${encodeURIComponent(target.id)}`, {
        method: "POST",
        headers: authHeaders(),
  ...
  const restoreBackupFile = async (target: BackupTarget, file: RemoteBackupFile) => {
    ...
    try {
      const response = await fetch(`/api/backup?action=restore&id=${encodeURIComponent(target.id)}`, {
        method: "POST",
        headers: authHeaders(),
  ...
  const deleteBackupFile = async (target: BackupTarget, file: RemoteBackupFile) => {
    ...
    try {
      const response = await fetch(`/api/backup?action=files&id=${encodeURIComponent(target.id)}`, {
        method: "DELETE",
        headers: authHeaders(),
        body: JSON.stringify({ key: file.key }),
      });
