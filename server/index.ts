...
import { deleteRemoteBackup, listRemoteBackups, normalizeBackupTarget, readRemoteBackup, runBackupTarget, runDueBackups, testBackupTarget, type BackupTargetConfig } from "./backup.js";
import { getExchangeRates } from "./exchangeRates.js";
import { sendNotificationDispatch, sendNotificationTest, type NotificationDispatchPayload } from "./notify.js";
...
if (!process.env.VERCEL) {
  ...
  setInterval(async () => {
    try {
      ...
      await writeDb(db);
      console.log(`YiHuoGe scheduled backups completed: ${results.length}`);
    } catch (error) {
      console.warn("YiHuoGe scheduled backup failed", error);
    }
  }, 10 * 60 * 1000);
}
