...
async function aiForgeAssets(text: string, ai: Database["ai"]): Promise<Asset[]> {
  ...
  const body = JSON.stringify({
    ...
    messages: [
      { role: "system", content: `?????????????????????????? CSV?TSV?JSON?????????????/???/?????????????????????

??? Asset ?????
- name: string?????????????????????????????
- type: ??? domain/vps/cloud/ai/membership/custom???=>domain?VPS/???/??/IP=>vps????/????/CDN/???=>cloud?OpenAI/Claude/Gemini/API/??=>ai???/??/SaaS=>membership?????=>custom?
- provider: string????/???/??/?????????????
- account: string???/??/IP/?? ID????????????
- renewalDate: YYYY-MM-DD????/???/????/expire/expiration/next billing date ?????????????????????
- price: number???/??/amount/cost????? 0?
- currency: string?? CNY/USD/HKD/JPY/EUR???? CNY?
- cycle: monthly/yearly/custom???=>monthly???=>yearly???=>custom?
- url: string?????/??/???/console/login/dashboard ????? url?
- tags: string[]????????AI????????
- notes: string????????????????????token?secret ?????????????????
